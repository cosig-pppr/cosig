...
\cosigsection{Multiple hypothesis correction}{24 June 2025}
...
\href{https://doi.org/10.1096/fba.2020-00047}{Teruya et al. (2020)} report on performing metabolomics to determine if there are any metabolites that are present at different abundance in the urine of elderly individuals versus young individuals. They perform hypothesis testing on 99 metabolites, finding 55 out of 99 to be significantly different at unadjusted $p < 0.05$. One would expect to obtain only around 5 false positives if all metabolites had no true difference between groups, yet the authors identified 55 metabolites with $p < 0.05$, indicating that it is likely that many of the findings are, in fact, true positives. Nonetheless, some of the resulting $p$ values might still be expected under the null hypothesis when considering how many tests were performed, so it is still important to use multiple test correction to ensure that any such low-evidence findings are filtered out.
